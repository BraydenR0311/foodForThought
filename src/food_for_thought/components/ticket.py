...
from .. import config
from ..common import MENU, QUOTE_DATA
from .food import Food
from .generic import Generic
from .text import QuoteSection, Text
...
class Ticket(pg.sprite.Sprite):
    ...
    def update(self, *args, **kwargs):
        ...
        if all(
            quote_sec.is_final_correct is not None for quote_sec in self.quote_sections
        ):
            self._finish()
    ...
    def _set_author_image(self, author_id) -> None:
        ...
        self.author_image = Generic(
            (config.IMAGE_DIR / "faces" / author_id).with_suffix(".jpg")
        )
